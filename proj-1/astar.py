...
Purpose:
The A star class is responsible for running the astar search on the board object
...
Object Methods:
    ...
    aStarSearch(self, board) -- Initializes the search by create tree and root and starts the astar search
...
class AStarSearch:
    ...
    def startSearch(self, tree):
        searchFileName = os.getcwd() + '/output/' + \
            str(tree.head.board.num) + '_atar_search.txt'
        ...
        with open(searchFileName, 'w') as search:
            ...
            while(len(tree.openlist) != 0 and self.counter < tree.head.board.maxL):
                ...
                if(currentNode.board.printState() not in tree.closedlist):
                    search.write(
                        str(currentNode.f) + ' ' + str(currentNode.depth) + ' ' + str(currentNode.h) + ' ' + currentNode.board.printState() + '\n')
